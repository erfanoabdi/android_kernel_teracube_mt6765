...
#else
	ret = consys_conninfra_on_power_ctrl_mt6895_gen(enable);
#endif /* MTK_CONNINFRA_CLOCK_BUFFER_API_AVAILABLE */
	if (enable)
		consys_update_ap2conn_hclk_mt6895_gen();
...
{
	return consys_conninfra_wakeup_mt6895_gen();
}
...
void consys_set_if_pinmux_mt6895(unsigned int enable)
{
	...
	if (enable) {
		...
		if (clock_type == CONNSYS_CLOCK_SCHEMATIC_26M_EXTCXO ||
			...
			consys_set_gpio_tcxo_mode_mt6895_gen(1, 1);
	#else
		...
		consys_set_if_pinmux_mt6895_gen(0);

		if (clock_type == CONNSYS_CLOCK_SCHEMATIC_26M_EXTCXO ||
			clock_type == CONNSYS_CLOCK_SCHEMATIC_52M_EXTCXO) {
	#if defined(CFG_CONNINFRA_ON_CTP)
			consys_set_gpio_tcxo_mode_mt6895_gen(1, 0);
	#else
			if (!IS_ERR(g_conninfra_pinctrl_ptr)) {
...
{
	return consys_polling_chipid_mt6895_gen(NULL);
}
...
static int connsys_adie_clock_buffer_setting(unsigned int curr_status, unsigned int next_status)
{
	...
	ret = connsys_adie_clock_buffer_setting_mt6895_gen(
...
int connsys_spi_master_cfg_mt6895(unsigned int next_status)
{
	connsys_wt_slp_top_ctrl_adie6637_mt6895_gen();
	return 0;
}
...
int connsys_a_die_cfg_mt6895(void)
{
	...
	int check = 0;
	struct consys_plat_thermal_data_mt6895 input;
	...
	memset(&input, 0, sizeof(struct consys_plat_thermal_data_mt6895));

	connsys_a_die_switch_to_gpio_mode_mt6895_gen();
	connsys_adie_top_ck_en_ctl_mt6895_gen(1);
	connsys_a_die_cfg_deassert_adie_reset_mt6895_gen();
	...
	check = connsys_a_die_cfg_read_adie_id_mt6895_gen(&adie_id, &hw_ver_id);
	if (check) {
	...
	connsys_a_die_switch_to_conn_mode_mt6895_gen();
...
{
	connsys_afe_sw_patch_mt6895_gen();
}
...
{
	return connsys_subsys_pll_initial_xtal_26000k_mt6895_gen();
}

int connsys_low_power_setting_mt6895(unsigned int curr_status, unsigned int next_status)
{
	connsys_adie_clock_buffer_setting(curr_status, next_status);

	if (curr_status == 0) {
		connsys_low_power_setting_mt6895_gen();
	} else {
		/* Not first on */
	}
	/* !!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!! */
...
int consys_sema_acquire_timeout_mt6895(unsigned int index, unsigned int usec)
{
	...
	pr_notice("Get semaphore 0x%x timeout, dump status:\n", index);
	pr_notice("M0:[0x%x] M1:[0x%x] M2:[0x%x] M3:[0x%x]\n",
...
int consys_spi_read_mt6895(enum sys_spi_subsystem subsystem, unsigned int addr, unsigned int *data)
{
	...
	ret = consys_spi_read_nolock_mt6895(subsystem, addr, data);
...
int consys_spi_write_mt6895(enum sys_spi_subsystem subsystem, unsigned int addr, unsigned int data)
{
	...
	ret = consys_spi_write_nolock_mt6895(subsystem, addr, data);
...
int consys_spi_clock_switch_mt6895(enum connsys_spi_speed_type type)
{
	...
	/* Get semaphore before read */
	if (consys_sema_acquire_timeout_mt6895(CONN_SEMA_RFSPI_INDEX, CONN_SEMA_TIMEOUT) == CONN_SEMA_GET_FAIL) {
		pr_notice("[SPI CLOCK SWITCH] Require semaphore fail\n");
		return -1;
	}
...
int consys_subsys_status_update_mt6895(bool on, int radio)
{
	...
	if (consys_sema_acquire_timeout_mt6895(CONN_SEMA_CONN_INFRA_COMMON_SYSRAM_INDEX, CONN_SEMA_TIMEOUT) == CONN_SEMA_GET_FAIL) {
		pr_notice("[%s] acquire semaphore (%d) timeout\n",
	...
	if (radio == CONNDRV_TYPE_FM) {
		if (consys_sema_acquire_timeout_mt6895(CONN_SEMA_RFSPI_INDEX, CONN_SEMA_TIMEOUT) == CONN_SEMA_GET_SUCCESS) {
	...
	unsigned int reg_offset, unsigned int value_offset, unsigned int size)
{
	...
	data2 = data & (~reg_mask);
	data2 = (data2 | value);
	consys_spi_write_nolock_mt6895(subsystem, addr, data2);
}
...
int connsys_adie_top_ck_en_ctl_mt6895(bool onoff)
{
	return connsys_adie_top_ck_en_ctl_mt6895_gen(onoff);
}
