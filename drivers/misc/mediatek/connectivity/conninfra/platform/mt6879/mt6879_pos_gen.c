 ...
 *
 * This file, mt6879_pos_gen.c was automatically generated
...
void consys_set_if_pinmux_mt6879_gen(unsigned int enable)
{
	...
	if (enable == 1) {
		/* set pinmux for the interface between D-die and A-die (Aux1) */
			...
			CONSYS_REG_WRITE_MASK(GPIO_REG_BASE +
				CONSYS_GEN_GPIO_MODE23_OFFSET_ADDR, 0x11000000, 0x77000000);
			CONSYS_REG_WRITE_MASK(GPIO_REG_BASE +
		...
		/* set pinmux driving to 4mA setting */
			...
			CONSYS_REG_WRITE_MASK(IOCFG_RT_REG_BASE +
				CONSYS_GEN_DRV_CFG2_OFFSET_ADDR, 0x8000, 0x38000);
		#endif

		/* set pinmux PUPD setting */
		#ifndef CONFIG_FPGA_EARLY_PORTING
			CONSYS_REG_WRITE_MASK(IOCFG_RT_REG_BASE +
				CONSYS_GEN_PU_CFG0_OFFSET_ADDR, 0x1080000, 0x1080000);
		#endif
	} else {
		/* set pinmux for the interface between D-die and A-die (Aux0) */
			...
			CONSYS_REG_WRITE_MASK(GPIO_REG_BASE +
				CONSYS_GEN_GPIO_MODE23_OFFSET_ADDR, 0x0, 0x77000000);
			CONSYS_REG_WRITE_MASK(GPIO_REG_BASE +
		...
		#ifndef CONFIG_FPGA_EARLY_PORTING
			CONSYS_REG_WRITE_MASK(IOCFG_RT_REG_BASE +
				CONSYS_GEN_PD_CFG0_OFFSET_ADDR, 0x1080000, 0x1080000);
		#endif
	}
}
...
void consys_set_gpio_tcxo_mode_mt6879_gen(unsigned int tcxo_mode, unsigned int enable)
{
	...
	if (enable == 1) {
		/* set GPIO168 pinmux for TCXO mode (Aux1) */
		#ifndef CONFIG_FPGA_EARLY_PORTING
			if (tcxo_mode == 1) {
				CONSYS_REG_WRITE_MASK(GPIO_REG_BASE +
					CONSYS_GEN_GPIO_MODE21_OFFSET_ADDR, 0x1, 0x7);
			}
		#endif
	} else {
		/* set GPIO168 pinmux for GPIO mode (Aux0) */
		#ifndef CONFIG_FPGA_EARLY_PORTING
			if (tcxo_mode == 1) {
				CONSYS_REG_WRITE_MASK(GPIO_REG_BASE +
					CONSYS_GEN_GPIO_MODE21_OFFSET_ADDR, 0x0, 0x7);
			}
		#endif
	}
}
...
void consys_update_ap2conn_hclk_mt6879_gen(void)
{
	...
	CONSYS_REG_WRITE(vir_addr_consys_gen_topckgen_base +
		CONSYS_GEN_CLK_CFG_16_CLR_OFFSET_ADDR, 0xF);

	/* set setting for connsys ap2conn clock */
	CONSYS_REG_WRITE(vir_addr_consys_gen_topckgen_base +
		CONSYS_GEN_CLK_CFG_16_SET_OFFSET_ADDR, 0x1);

	/* toggle setting for connsys ap2conn clock */
	CONSYS_REG_WRITE(vir_addr_consys_gen_topckgen_base +
		CONSYS_GEN_CLK_CFG_UPDATE2_OFFSET_ADDR, 0x4);
...
int connsys_a_die_switch_to_gpio_mode_mt6879_gen(void)
{
	...
	if ((consys_co_clock_type_mt6879() == CONNSYS_CLOCK_SCHEMATIC_26M_COTMS) ||
		(consys_co_clock_type_mt6879() == CONNSYS_CLOCK_SCHEMATIC_26M_EXTCXO)) {
		/* CONN_WF_CTRL2 swtich to GPIO mode, GPIO output value low */
		#ifndef CONFIG_FPGA_EARLY_PORTING
			if (consys_get_adie_chipid_mt6879() == ADIE_6637) {
				CONSYS_CLR_BIT(GPIO_REG_BASE +
					CONSYS_GEN_GPIO_DOUT6_OFFSET_ADDR, (0x1 << 6));
				CONSYS_SET_BIT(GPIO_REG_BASE +
					CONSYS_GEN_GPIO_DIR6_OFFSET_ADDR, (0x1 << 6));
				CONSYS_REG_WRITE_MASK(GPIO_REG_BASE +
					CONSYS_GEN_GPIO_MODE24_OFFSET_ADDR, 0x0, 0x7000000);
			}
		#endif
	} else if ((consys_co_clock_type_mt6879() == CONNSYS_CLOCK_SCHEMATIC_52M_COTMS) ||
		(consys_co_clock_type_mt6879() == CONNSYS_CLOCK_SCHEMATIC_52M_EXTCXO)) {
...
int connsys_a_die_cfg_adie6637_PART1_mt6879_gen(void)
{
	...
	#ifndef CONFIG_FPGA_EARLY_PORTING
		consys_spi_write_nolock_mt6879(SYS_SPI_TOP, CONSYS_GEN_ADIE6637_ATOP_WRI_CTR2, 0x7700);
	#endif

	/* set WF low power cmd as DBDC mode & legacy interface */
	#ifndef CONFIG_FPGA_EARLY_PORTING
		consys_spi_write_nolock_mt6879(SYS_SPI_TOP, CONSYS_GEN_ADIE6637_ATOP_SMCTK11, 0x21);
	#endif
...
int connsys_a_die_efuse_read_adie6637_check_efuse_valid_mt6879_gen(bool *pefuse_valid)
{
	...
	/* Efuse control clear, clear Status /trigger */
		...
		consys_spi_write_nolock_mt6879(SYS_SPI_TOP, CONSYS_GEN_ADIE6637_ATOP_EFUSE_CTRL, efuse_ctrl);
	#endif
	...
	#ifndef CONFIG_FPGA_EARLY_PORTING
		retry = 0;
		consys_spi_read_nolock_mt6879(SYS_SPI_TOP, CONSYS_GEN_ADIE6637_ATOP_EFUSE_CTRL, &ret);
		...
		unsigned int *pefuse0)
{
	...
	#ifndef CONFIG_FPGA_EARLY_PORTING
		if (efuse_valid) {
			check = consys_spi_read_nolock_mt6879(SYS_SPI_TOP, CONSYS_GEN_ADIE6637_ATOP_EFUSE_RDATA0, &efuse_rdata0);
			if (check)
				return check;
			if (pefuse0 != NULL)
		...
		unsigned int *pefuse1)
{
	...
	#ifndef CONFIG_FPGA_EARLY_PORTING
		if (efuse_valid) {
			check = consys_spi_read_nolock_mt6879(SYS_SPI_TOP, CONSYS_GEN_ADIE6637_ATOP_EFUSE_RDATA1, &efuse_rdata1);
			if (check)
				return check;
			if (pefuse1 != NULL)
		...
		unsigned int *pefuse2)
{
	...
	#ifndef CONFIG_FPGA_EARLY_PORTING
		if (efuse_valid) {
			check = consys_spi_read_nolock_mt6879(SYS_SPI_TOP, CONSYS_GEN_ADIE6637_ATOP_EFUSE_RDATA2, &efuse_rdata2);
			if (check)
				return check;
			if (pefuse2 != NULL)
		...
		unsigned int *pefuse3)
{
	...
	#ifndef CONFIG_FPGA_EARLY_PORTING
		if (efuse_valid) {
			check = consys_spi_read_nolock_mt6879(SYS_SPI_TOP, CONSYS_GEN_ADIE6637_ATOP_EFUSE_RDATA3, &efuse_rdata3);
			if (check)
				return check;
			if (pefuse3 != NULL)
		...
		int *poffset)
{
	...
	/* replace CR default value by Efuse Value */
		...
		if ((efuse_valid) && ((pefuse_list != NULL) && (efuse_size > 1) && (pefuse_list[1] & (0x1 << 7)))) {
			consys_spi_write_offset_range_nolock_mt6879(
...
int connsys_a_die_cfg_adie6637_PART2_mt6879_gen(unsigned int hw_ver_id)
{
	...
	/* disable CAL LDO */
	#ifndef CONFIG_FPGA_EARLY_PORTING
		consys_spi_write_nolock_mt6879(SYS_SPI_TOP, CONSYS_GEN_ADIE6637_ATOP_RG_WF2_POS_01, 0xE8002);
	#endif

	/* disable CAL LDO */
	#ifndef CONFIG_FPGA_EARLY_PORTING
		consys_spi_write_nolock_mt6879(SYS_SPI_TOP, CONSYS_GEN_ADIE6637_ATOP_RG_WF2_POS_02, 0xE8002);
	#endif

	/* initial IR value for WF0 THADC */
	#ifndef CONFIG_FPGA_EARLY_PORTING
		consys_spi_write_nolock_mt6879(SYS_SPI_TOP, CONSYS_GEN_ADIE6637_ATOP_RG_WF2_TIME_COUNT_01, 0x2008);
	#endif

	/* initial IR value for WF1 THADC */
	#ifndef CONFIG_FPGA_EARLY_PORTING
		consys_spi_write_nolock_mt6879(SYS_SPI_TOP, CONSYS_GEN_ADIE6637_ATOP_RG_WF2_TIME_COUNT_02, 0x2008);
	#endif

	/* DA_WF0_SW_RFDIGLDO Wakeup Sleep Sequence Update */
	#ifndef CONFIG_FPGA_EARLY_PORTING
		consys_spi_write_nolock_mt6879(SYS_SPI_TOP, CONSYS_GEN_ADIE6637_ATOP_RG_WF1_RFLDO_TIME, 0xF000B);
	#endif

	/* DA_WF1_SW_RFDIGLDO Wakeup Sleep Sequence Update */
	#ifndef CONFIG_FPGA_EARLY_PORTING
		consys_spi_write_nolock_mt6879(SYS_SPI_TOP, CONSYS_GEN_ADIE6637_ATOP_RG_WF1_RFDIG_OFF_TIME, 0xC000C);
	#endif

	/* set DA_BT0_BG_EN_RFDIG On/Off time counter */
	#ifndef CONFIG_FPGA_EARLY_PORTING
		consys_spi_write_nolock_mt6879(SYS_SPI_TOP, CONSYS_GEN_ADIE6637_ATOP_RG_BT0_BG_TIME, 0x20003);
	#endif

	/* set bt_iso_en time counter */
	#ifndef CONFIG_FPGA_EARLY_PORTING
		consys_spi_write_nolock_mt6879(SYS_SPI_TOP, CONSYS_GEN_ADIE6637_ATOP_RG_BT0_ISO_TIME, 0x1000B);
	#endif

	/* set bt_xobuf_time counter */
	#ifndef CONFIG_FPGA_EARLY_PORTING
		consys_spi_write_nolock_mt6879(SYS_SPI_TOP, CONSYS_GEN_ADIE6637_ATOP_RG_BT0_XOBUF_TIME, 0x10004);
	#endif

	/* set bt_ck_en time counter */
	#ifndef CONFIG_FPGA_EARLY_PORTING
		consys_spi_write_nolock_mt6879(SYS_SPI_TOP, CONSYS_GEN_ADIE6637_ATOP_RG_BT0_CKEN_TIME, 0x20003);
	#endif

	/* set BT_WAKEUPSLEEP_PON_OFF counter Threshold to 8us */
	#ifndef CONFIG_FPGA_EARLY_PORTING
		consys_spi_write_nolock_mt6879(SYS_SPI_TOP, CONSYS_GEN_ADIE6637_ATOP_RG_WAKEUPSLEEP_PON_OFF, 0x150008);
	#endif

	/* Update WF XOBUF disable, WF sys will enable XOBUF when WF Power On */
	#ifndef CONFIG_FPGA_EARLY_PORTING
		consys_spi_write_nolock_mt6879(SYS_SPI_TOP, CONSYS_GEN_ADIE6637_ATOP_RG_TOP_XO_02, 0x88000000);
	#endif

	/* Update WF XOBUF disable, WF sys will enable XOBUF when WF Power On */
	#ifndef CONFIG_FPGA_EARLY_PORTING
		consys_spi_write_nolock_mt6879(SYS_SPI_TOP, CONSYS_GEN_ADIE6637_ATOP_RG_TOP_XO_03, 0x0);
	#endif

	/* Update BT/FM/GPS XOBUF disable, this will use drvier to config in BGF platform */
	#ifndef CONFIG_FPGA_EARLY_PORTING
		consys_spi_write_nolock_mt6879(SYS_SPI_TOP, CONSYS_GEN_ADIE6637_ATOP_RG_TOP_XO_07, 0x0);
	#endif

	/* Update BT AIQ SW to normal mode */
	#ifndef CONFIG_FPGA_EARLY_PORTING
		consys_spi_write_nolock_mt6879(SYS_SPI_TOP, CONSYS_GEN_ADIE6637_ATOP_RG_ENCAL_WFBT_IF_SW_01, 0x5);
	#endif

	/* Update to always Power On FMSYS in ADIE */
	#ifndef CONFIG_FPGA_EARLY_PORTING
		consys_spi_write_nolock_mt6879(SYS_SPI_TOP, CONSYS_GEN_ADIE6637_ATOP_RG_FMCTL4, 0x35);
	#endif

	/* BT/TOP SPI CK PU/PD no pull after hw POS */
	#ifndef CONFIG_FPGA_EARLY_PORTING
		consys_spi_write_nolock_mt6879(SYS_SPI_TOP, CONSYS_GEN_ADIE6637_ATOP_RG_TOP_PULL, 0xA6AAAA44);
	#endif

	if (hw_ver_id == 0x8a01) {
		/* Update CR of VCONST_BG WF0 Only For E2 Update */
		#ifndef CONFIG_FPGA_EARLY_PORTING
			consys_spi_write_nolock_mt6879(SYS_SPI_TOP, CONSYS_GEN_ADIE6637_ATOP_RG_WF0_BG, 0xE008);
		#endif

		/* Update CR of VCONST_BG WF1 Only For E2 Update */
		#ifndef CONFIG_FPGA_EARLY_PORTING
			consys_spi_write_nolock_mt6879(SYS_SPI_TOP, CONSYS_GEN_ADIE6637_ATOP_RG_WF1_BG, 0xE008);
		#endif
	}

	/* EN WF0 CAL/TRX LDO bleeding current both update for E1 and E2 POS */
	#ifndef CONFIG_FPGA_EARLY_PORTING
		consys_spi_write_nolock_mt6879(SYS_SPI_TOP, CONSYS_GEN_ADIE6637_ATOP_RG_WF0_TOP_01, 0xE440A);
	#endif

	/* EN WF1 CAL/TRX LDO bleeding current both update for E1 and E2 POS */
	#ifndef CONFIG_FPGA_EARLY_PORTING
		consys_spi_write_nolock_mt6879(SYS_SPI_TOP, CONSYS_GEN_ADIE6637_ATOP_RG_WF1_TOP_01, 0xE401A);
	#endif
...
{
	if (consys_get_adie_chipid_mt6879() == ADIE_6637)
		connsys_a_die_cfg_adie6637_deassert_adie_reset_mt6879_gen();
	else
		return -1;
...
int connsys_a_die_cfg_read_adie_id_mt6879_gen(unsigned int *padie_id, unsigned int *phw_ver_id)
{
	...
	if (consys_get_adie_chipid_mt6879() == ADIE_6637)
		check = connsys_a_die_cfg_adie6637_read_adie_id_mt6879_gen(padie_id, phw_ver_id);
	else
		return -1;
		...
		int *poffset)
{
	...
	if (consys_get_adie_chipid_mt6879() == ADIE_6637) {
		connsys_a_die_cfg_adie6637_PART1_mt6879_gen();
		connsys_a_die_efuse_read_adie6637_check_efuse_valid_mt6879_gen(&efuse_valid);
		connsys_a_die_efuse_read_adie6637_get_efuse_count_mt6879_gen(&efuse_size);
		...
		ret[0] = connsys_a_die_efuse_read_adie6637_get_efuse0_info_mt6879_gen(efuse_valid, &(efuse_list[0]));
		CONSYS_REG_WRITE(psysram_efuse_list[0], efuse_list[0]);

		ret[1] = connsys_a_die_efuse_read_adie6637_get_efuse1_info_mt6879_gen(efuse_valid, &(efuse_list[1]));
		CONSYS_REG_WRITE(psysram_efuse_list[1], efuse_list[1]);

		ret[2] = connsys_a_die_efuse_read_adie6637_get_efuse2_info_mt6879_gen(efuse_valid, &(efuse_list[2]));
...
{
	if (consys_get_adie_chipid_mt6879() == ADIE_6637)
		connsys_a_die_cfg_adie6637_PART2_mt6879_gen(hw_ver_id);
	else
		return -1;
...
int connsys_a_die_switch_to_conn_mode_mt6879_gen(void)
{
	...
	#ifndef CONFIG_FPGA_EARLY_PORTING
		CONSYS_REG_WRITE_MASK(GPIO_REG_BASE +
			CONSYS_GEN_GPIO_MODE24_OFFSET_ADDR, 0x1000000, 0x7000000);
	#endif
		...
		unsigned int spi_semaphore_timeout_usec)
{
	...
	/* set WF_PAD to HighZ */
		...
		consys_spi_write_nolock_mt6879(SYS_SPI_TOP, CONSYS_GEN_ATOP_RG_ENCAL_WBTAC_IF_SW, 0x80000000);

		consys_sema_release_mt6879(spi_semaphore_index);
	#endif
	...
	/* restore CR initial value */
		...
		consys_spi_write_nolock_mt6879(SYS_SPI_TOP, CONSYS_GEN_ATOP_RG_ENCAL_WBTAC_IF_SW, 0x5);
...
int connsys_low_power_setting_mt6879_gen(void)
{
	...
	for (i = 0; i < 1024; i++) {
		CONSYS_REG_WRITE(vir_addr_consys_gen_conn_infra_sysram_base_offset +
			addr_offset, g_cmdbt_dwn_value_ary_mt6879[i]);
		addr_offset += 0x4;
	}
...
int consys_conninfra_wakeup_mt6879_gen(void)
{
	...
	/* (polling "10 times" for specific project code and each polling interval is "1ms") */
	if (consys_polling_chipid_mt6879_gen(NULL))
		return -1;
		...
		unsigned int spi_semaphore_timeout_usec)
{
	...
	if (consys_get_adie_chipid_mt6879() == ADIE_6637) {
		...
		#ifndef CONFIG_FPGA_EARLY_PORTING
			if (wifi_on == 1) {
				consys_spi_write_nolock_mt6879(SYS_SPI_TOP, CONSYS_GEN_ATOP_RG_TOP_XO_2, 0xFE000000);
				consys_spi_write_nolock_mt6879(SYS_SPI_TOP, CONSYS_GEN_ATOP_RG_TOP_XO_3, 0xE0000000);
			}
		#endif
		...
		/* turn on GPS/BT/FM XO_BUF */
			...
			if (bt_on == 1 || gps_on == 1 || fm_on == 1) {
				consys_spi_write_nolock_mt6879(SYS_SPI_TOP, CONSYS_GEN_ATOP_RG_TOP_XO_7, 0x7);
			}
		#endif
		...
		#ifndef CONFIG_FPGA_EARLY_PORTING
			if (wifi_on == 0) {
				consys_spi_write_nolock_mt6879(SYS_SPI_TOP, CONSYS_GEN_ATOP_RG_TOP_XO_2, 0x88000000);
				consys_spi_write_nolock_mt6879(SYS_SPI_TOP, CONSYS_GEN_ATOP_RG_TOP_XO_3, 0x0);
			}
		#endif
		...
		/* turn off GPS/BT/FM XO_BUF */
			...
			if (bt_gps_fm_off == true) {
				consys_spi_write_nolock_mt6879(SYS_SPI_TOP, CONSYS_GEN_ATOP_RG_TOP_XO_7, 0x0);
			}
		#endif
