...
static INT32 consys_clock_register_device(VOID);
static PVOID consys_clock_get_regmap(VOID);
...
static int consys_clock_mt6685_probe(struct platform_device *pdev);
...
static struct regulator *reg_VRFIO18;
...
WMT_CONSYS_IC_OPS consys_ic_ops_mt6855 = {
	...
	.consys_ic_clock_register_device = consys_clock_register_device,
	.consys_ic_clock_get_regmap = consys_clock_get_regmap,
...
static struct platform_driver consys_pmic_mt6363_dev_drv = {
	.probe = consys_pmic_mt6363_probe,
	.driver = {
		.name = "mt6363-consys",
#ifdef CONFIG_OF
...
static struct platform_driver consys_pmic_mt6369_dev_drv = {
	.probe = consys_pmic_mt6369_probe,
	.driver = {
		.name = "mt6369-consys",
#ifdef CONFIG_OF
...
static struct platform_driver consys_clock_mt6685_dev_drv = {
	.probe = consys_clock_mt6685_probe,
	.driver = {
		.name = "mt6685-consys",
#ifdef CONFIG_OF
...
#define JTAG_ADDR2_BASE 0x11d40000
#endif
				  ...
				  void *unused)
{
	...
	oc_counter++;
	WMT_PLAT_PR_INFO("[%s] VCN13 OC times: %d\n", __func__, oc_counter);
				  ...
				  void *unused)
{
	...
	oc_counter++;
	WMT_PLAT_PR_INFO("[%s] VRFIO18 OC times: %d\n", __func__, oc_counter);
						...
						unsigned int mask, unsigned int value)
{
	...
	if (!rmap) {
		WMT_PLAT_PR_ERR("%s regmap is NULL\n", __func__);
		return;
	}
...
static int consys_pmic_mt6363_probe(struct platform_device *pdev)
{
	...
	if (!g_regmap_mt6363)
		WMT_PLAT_PR_ERR("%s failed to get g_regmap_mt6363\n", __func__);
	else
		WMT_PLAT_PR_INFO("%s get regmap_mt6363 success!!\n", __func__);
...
static int consys_pmic_mt6369_probe(struct platform_device *pdev)
{
	...
	if (!g_regmap_mt6369)
		WMT_PLAT_PR_ERR("%s failed to get g_regmap_mt6369\n", __func__);
	else
		WMT_PLAT_PR_INFO("%s get regmap_mt6369 success!!\n", __func__);
...
static int consys_clock_mt6685_probe(struct platform_device *pdev)
{
	...
	if (!g_regmap_mt6685) {
		WMT_PLAT_PR_ERR("%s failed to get g_regmap_mt6685\n", __func__);
		return 0;
	}
...
static INT32 consys_clock_register_device(VOID)
{
	...
	ret = platform_driver_register(&consys_clock_mt6685_dev_drv);
	if (ret)
		WMT_PLAT_PR_ERR("Conninfra clock ic mt6685 driver registered failed(%d)\n", ret);
	else
		WMT_PLAT_PR_INFO("%s mt6685 ok.\n", __func__);
...
static INT32 consys_pmic_register_device(VOID)
{
	...
	ret = platform_driver_register(&consys_pmic_mt6363_dev_drv);
	if (ret)
		WMT_PLAT_PR_ERR("WMT pmic mt6363 driver registered failed(%d)\n", ret);
	else
		WMT_PLAT_PR_INFO("%s mt6363 ok.\n", __func__);

	ret = platform_driver_register(&consys_pmic_mt6369_dev_drv);
	if (ret)
		WMT_PLAT_PR_ERR("WMT pmic mt6369 driver registered failed(%d)\n", ret);
	else
		WMT_PLAT_PR_INFO("%s mt6369 ok.\n", __func__);
#endif
...
static VOID consys_hw_reset_bit_set(MTK_WCN_BOOL enable)
{
	consys_hw_reset_bit_set_mt6855_gen(enable);
}
...
static INT32 consys_polling_goto_idle(VOID)
{
	 ...
	 */
	iRet = consys_polling_goto_idle_mt6855_gen(&consys_ver_id);
...
static VOID consys_hw_spm_clk_gating_enable(VOID)
{
	consys_hw_spm_clk_gating_enable_mt6855_gen();
}

int consys_platform_spm_conn_ctrl_mt6855(unsigned int enable)
{
	int ret = 0;
	struct platform_device *pdev = get_consys_device();

	if (!pdev) {
		WMT_PLAT_PR_ERR("get_consys_device fail.\n");
		return -1;
	}
...
static INT32 consys_hw_power_ctrl(MTK_WCN_BOOL enable)
{
	...
	if (enable) {
		...
		if (iRet) {
			WMT_PLAT_PR_INFO("consys_platform_spm_conn_ctrl_mt6855(1) fail(%d)\n", iRet);
			return iRet;
		}

		if (conn_reg.topckgen_base != 0) {
			/* Clear WIFI Active / WIFI CCIF4 Ready / WIFI Power */
...
#else
		consys_hw_power_ctrl_mt6855_gen(true);
		...
		iRet = consys_platform_spm_conn_ctrl_mt6855(0);
		if (iRet) {
			WMT_PLAT_PR_INFO("consys_platform_spm_conn_ctrl_mt6855(0) fail(%d)\n", iRet);
			return iRet;
		}
...
static INT32 polling_consys_chipid(VOID)
{
	...
	check = polling_consys_chipid_mt6855_gen(&consys_ver_id, &consys_fw_ver);
	if (check == 0) {
...
static VOID consys_set_access_emi_hw_mode(VOID)
{
	consys_set_access_emi_hw_mode_mt6855_gen();
}
...
static VOID consys_emi_entry_address(VOID)
{
	consys_emi_entry_address_mt6855_gen();
}

static VOID consys_set_xo_osc_ctrl(VOID)
{
	consys_set_xo_osc_ctrl_mt6855_gen();
}

static VOID consys_identify_adie(VOID)
{
	consys_identify_adie_mt6855_gen();
}

static VOID consys_wifi_ctrl_setting(VOID)
{
	consys_wifi_ctrl_setting_mt6855_gen();
}

static VOID consys_wifi_ctrl_switch_conn_mode(VOID)
{
	consys_wifi_ctrl_switch_conn_mode_mt6855_gen();
}

static VOID consys_bus_timeout_config(VOID)
{
	consys_bus_timeout_config_mt6855_gen();
}
...
static VOID consys_hw_vcn33_primary_rc_mode_enable(VOID)
{
	...
	int ret = 0;
     ...
     /* 1. set PMIC VCN33_1 LDO PMIC HW mode control by PMRC_EN[9][8][7][6] */
     /* 2. set PMIC VCN33_1 LDO op_mode = 0 */
     /* 3. set PMIC VCN33_1 LDO SW_OP_EN = 1, SW_EN = 1, SW_LP =0 (sw enable & normal mode)*/
	...
	if (reg_VCN33_1) {
		...
		if (ret)
			WMT_PLAT_PR_ERR("Enable VCN33_1. ret=%d\n", ret);
	}

	consys_pmic_regmap_set_value(r, MT6369_RG_LDO_VCN33_1_RC9_OP_MODE_ADDR, 1 << 1, 0 << 1);
	consys_pmic_regmap_set_value(r, MT6369_RG_LDO_VCN33_1_RC9_OP_EN_ADDR,   1 << 1, 1 << 1);
	consys_pmic_regmap_set_value(r, MT6369_RG_LDO_VCN33_1_RC9_OP_CFG_ADDR,  1 << 1, 0 << 1);
	consys_pmic_regmap_set_value(r, MT6369_RG_LDO_VCN33_1_RC8_OP_MODE_ADDR, 1 << 0, 0 << 0);
	consys_pmic_regmap_set_value(r, MT6369_RG_LDO_VCN33_1_RC8_OP_EN_ADDR,   1 << 0, 1 << 0);
	consys_pmic_regmap_set_value(r, MT6369_RG_LDO_VCN33_1_RC8_OP_CFG_ADDR,  1 << 0, 0 << 0);
	consys_pmic_regmap_set_value(r, MT6369_RG_LDO_VCN33_1_RC7_OP_MODE_ADDR, 1 << 7, 0 << 7);
	consys_pmic_regmap_set_value(r, MT6369_RG_LDO_VCN33_1_RC7_OP_EN_ADDR,   1 << 7, 1 << 7);
	consys_pmic_regmap_set_value(r, MT6369_RG_LDO_VCN33_1_RC7_OP_CFG_ADDR,  1 << 7, 0 << 7);
	consys_pmic_regmap_set_value(r, MT6369_RG_LDO_VCN33_1_RC6_OP_MODE_ADDR, 1 << 6, 0 << 6);
	consys_pmic_regmap_set_value(r, MT6369_RG_LDO_VCN33_1_RC6_OP_EN_ADDR,   1 << 6, 1 << 6);
	consys_pmic_regmap_set_value(r, MT6369_RG_LDO_VCN33_1_RC6_OP_CFG_ADDR,  1 << 6, 0 << 6);
}
#endif
...
static VOID consys_hw_vcn33_primary_legacy_mode_enable(VOID)
{
	...
	/* 2. set PMIC VCN33_1 LDO op_mode = 1 */
	/* 3. set PMIC VCN33_1 LDO SW_OP_EN = 1, SW_EN = 1, SW_LP =0 (sw enable & normal mode)*/
	...
	if (reg_VCN33_1) {
		...
		ret = regulator_enable(reg_VCN33_1); /* SW_EN = 1 */
		if (ret)
			WMT_PLAT_PR_ERR("Enable VCN33_1 fail. ret=%d\n", ret);
	}
	consys_pmic_regmap_set_value(r, MT6369_RG_LDO_VCN33_1_HW0_OP_EN_ADDR,   1 << 0, 1 << 0);
	consys_pmic_regmap_set_value(r, MT6369_RG_LDO_VCN33_1_HW0_OP_CFG_ADDR,  1 << 0, 1 << 0);
	consys_pmic_regmap_set_value(r, MT6369_RG_LDO_VCN33_1_HW0_OP_MODE_ADDR, 1 << 0, 1 << 0);
}
#endif
...
static VOID consys_hw_vcn13_secondary_rc_mode_enable(VOID)
{
	...
	/* 2. set PMIC VCN13 LDO op_mode = 0 */
	/* 3. set PMIC VCN13 LDO SW_OP_EN = 1, SW_EN = 1, SW_LP =0 (sw enable & normal mode)*/
	/* 4. set PMIC VCN13 LDO HW_OP_EN = 1, HW_OP_CFG = 0 */
	if (reg_VCN13) {
		/* set PMIC VCN13 LDO 1.3V */
		regulator_set_voltage(reg_VCN13, 1300000, 1300000);
		/* set PMIC VCN13 LDO SW_OP_EN = 1, SW_EN = 1, SW_LP = 0 (sw enable & normal mode) */
		regulator_set_mode(reg_VCN13, REGULATOR_MODE_NORMAL); /* SW_LP = 0 */
		ret = regulator_enable(reg_VCN13); /* SW_EN = 1 */
		if (ret)
			WMT_PLAT_PR_ERR("Enable VCN13 fail. ret=%d\n", ret);
	}
...
#if CONSYS_PMIC_CTRL_ENABLE
static VOID consys_hw_vcn33_secondary_rc_mode_enable(VOID)
{
	struct regmap *r = g_regmap_mt6369;
	int ret = 0;
	...
	/* 2. set PMIC VCN33_1 LDO op_mode = 0 */
	/* 3. set PMIC VCN33_1 LDO SW_OP_EN = 0, SW_EN = 0, SW_LP =0 (sw disable)*/
	/* 4. set PMIC VCN33_1 LDO HW_OP_EN = 1, HW_OP_CFG = 0 */
	if (reg_VCN33_1) {
		...
		regulator_set_mode(reg_VCN33_1, REGULATOR_MODE_NORMAL); /* SW_LP = 0 */
		if (ret)
			WMT_PLAT_PR_ERR("disable VCN33_1 fail. ret=%d\n", ret);
	}
	...
	/* 2. set PMIC VCN33_2 LDO op_mode = 0 */
	/* 3. set PMIC VCN33_2 LDO SW_OP_EN = 0, SW_EN = 0, SW_LP =0 (sw disable)*/
	/* 4. set PMIC VCN33_2 LDO HW_OP_EN = 1, HW_OP_CFG = 0 */
	if (reg_VCN33_2) {
		/* set PMIC VCN33_2 LDO SW_OP_EN = 0, SW_EN = 0, SW_LP = 0 (SW disable) */
		regulator_set_mode(reg_VCN33_2, REGULATOR_MODE_NORMAL); /* SW_LP = 0 */
		if (ret)
			WMT_PLAT_PR_ERR("disable VCN33_2 fail. ret=%d\n", ret);
	}

	consys_pmic_regmap_set_value(r, MT6369_RG_LDO_VCN33_2_RC8_OP_MODE_ADDR, 1 << 0, 0 << 0);
	consys_pmic_regmap_set_value(r, MT6369_RG_LDO_VCN33_2_RC8_OP_EN_ADDR,   1 << 0, 1 << 0);
	consys_pmic_regmap_set_value(r, MT6369_RG_LDO_VCN33_2_RC8_OP_CFG_ADDR,  1 << 0, 0 << 0);
}
#endif
...
static VOID consys_hw_vcn13_secondary_legacy_mode_enable(VOID)
{
	...
	/* 2. set PMIC VCN13 LDO op_mode = 1 */
	/* 3. set PMIC VCN13 LDO SW_OP_EN = 1, SW_EN = 1, SW_LP =0 (sw enable & normal mode)*/
	/* 4. set PMIC VCN13 LDO HW_OP_EN = 1, HW_OP_CFG = 1 */
	if (reg_VCN13) {
		/* set PMIC VCN13 LDO 1.8V */
		regulator_set_voltage(reg_VCN13, 1300000, 1300000);
		/* set PMIC VCN13 LDO SW_OP_EN = 1, SW_EN = 1, SW_LP = 0 (sw enable & normal mode) */
		regulator_set_mode(reg_VCN13, REGULATOR_MODE_NORMAL); /* SW_LP = 0 */
		ret = regulator_enable(reg_VCN13); /* SW_EN = 1 */
	...
	consys_pmic_regmap_set_value(r, MT6363_RG_LDO_VCN13_HW0_OP_EN_ADDR,   1 << 0, 1 << 0);
	consys_pmic_regmap_set_value(r, MT6363_RG_LDO_VCN13_HW0_OP_CFG_ADDR,  1 << 0, 1 << 0);
...
static INT32 consys_hw_vcn18_ctrl(MTK_WCN_BOOL enable)
{
	...
	if (enable) {
		...
		if (consys_is_rc_mode_enable_mt6855()) {
			...
			/* 2. set PMIC VRFIO18 LDO op_mode = 0 */
			/* 3. set PMIC VRFIO18 LDO SW_OP_EN = 1, SW_EN = 1, SW_LP =0 (sw enable & normal mode)*/
			/* 4. set PMIC VRFIO18 LDO HW_OP_EN = 1, HW_OP_CFG = 0 */
			if (reg_VRFIO18) {
				/* set PMIC VRFIO18 LDO 1.8V */
				regulator_set_voltage(reg_VRFIO18, 1800000, 1800000);
				/* set PMIC VRFIO18 LDO SW_OP_EN = 1, SW_EN = 1, SW_LP = 0 (sw enable & normal mode) */
				regulator_set_mode(reg_VRFIO18, REGULATOR_MODE_NORMAL); /* SW_LP = 0 */
				ret = regulator_enable(reg_VRFIO18); /* SW_EN = 1 */
				if (ret)
					WMT_PLAT_PR_ERR("Enable VRFIO18 fail. ret=%d\n", ret);
			}
			...
			if (mtk_wcn_consys_get_adie_chipid() == PRIMARY_ADIE) {
				...
				consys_hw_vcn33_secondary_rc_mode_enable();
			}
		} else {
			/* Legacy mode */
			/* 1. set PMIC VRFIO18 LDO PMIC HW mode control by SRCCLKENA0 */
			/* 2. set PMIC VRFIO18 LDO op_mode = 1 */
			/* 3. set PMIC VRFIO18 LDO SW_OP_EN = 1, SW_EN = 1, SW_LP =0 (sw enable & normal mode)*/
			/* 4. set PMIC VRFIO18 LDO HW_OP_EN = 1, HW_OP_CFG = 1 */
			if (reg_VRFIO18) {
				/* set PMIC VRFIO18 LDO 1.8V */
				regulator_set_voltage(reg_VRFIO18, 1800000, 1800000);
				/* set PMIC VRFIO18 LDO SW_OP_EN = 1, SW_EN = 1, SW_LP = 0 (sw enable & normal mode) */
		...
		if (mtk_wcn_consys_get_adie_chipid() == SECONDARY_ADIE) {
			...
			if (reg_VCN13) {
				if (regulator_disable(reg_VCN13))
					WMT_PLAT_PR_ERR("disable VCN13 fail!\n");
			}
			/* delay 300us */
			udelay(300);
		}

		WMT_PLAT_PR_INFO("Turn off VRFIO18\n");
		if (reg_VRFIO18) {
			if (regulator_disable(reg_VRFIO18))
				WMT_PLAT_PR_ERR("disable VRFIO18 fail!\n");
		}

		if (mtk_wcn_consys_get_adie_chipid() == PRIMARY_ADIE) {
			/* delay 300us */
			udelay(300);
			WMT_PLAT_PR_INFO("Turn off VCN33_1\n");
			if (reg_VCN33_1) {
				if (regulator_disable(reg_VCN33_1))
					WMT_PLAT_PR_ERR("disable VCN33_1 fail!\n");
			}
		}
	}
#endif
...
static INT32 consys_hw_vcn28_ctrl(UINT32 enable)
{
	...
	if (consys_is_rc_mode_enable_mt6855()) {
		...
		if (enable) {
			...
			/* 2. set PMIC VCN33_2 LDO op_mode = 1 */
			/* 3. set PMIC VCN33_2 LDO SW_OP_EN = 1, SW_EN = 1, SW_LP =0 (sw enable & normal mode)*/
			/* 4. set PMIC VCN33_2 LDO HW_OP_EN = 1, HW_OP_CFG = 0 */
			if (reg_VCN33_2) {
				/* set PMIC VCN33_2 LDO 2.8V */
				regulator_set_voltage(reg_VCN33_2, 2800000, 2800000);
				/* set PMIC VCN33_2 LDO SW_OP_EN = 1, SW_EN = 1, SW_LP = 0 (sw enable & normal mode) */
				regulator_set_mode(reg_VCN33_2, REGULATOR_MODE_NORMAL); /* SW_LP = 0 */
				ret = regulator_enable(reg_VCN33_2); /* SW_EN = 1 */
				if (ret)
					WMT_PLAT_PR_ERR("Enable VCN33_2 fail. ret=%d\n", ret);
			}
			consys_pmic_regmap_set_value(r, MT6369_RG_LDO_VCN33_2_HW0_OP_EN_ADDR,   1 << 0, 1 << 0);
			consys_pmic_regmap_set_value(r, MT6369_RG_LDO_VCN33_2_HW0_OP_CFG_ADDR,  1 << 0, 0 << 0);
			consys_pmic_regmap_set_value(r, MT6369_RG_LDO_VCN33_2_HW0_OP_MODE_ADDR, 1 << 0, 1 << 0);
		} else {
			if (reg_VCN33_2) {
				if (regulator_disable(reg_VCN33_2))
					WMT_PLAT_PR_ERR("disable VCN33_2 fail!\n");
			}
		}
	}
...
#if CONSYS_PMIC_CTRL_ENABLE
static INT32 consys_hw_wifi_bt_legacy_mode_vcn33_1_ctrl(UINT32 enable)
{
	struct regmap *r = g_regmap_mt6369;
	...
	WMT_PLAT_PR_INFO("%s enable_count %d\n", __func__, enable_count);
	if (enable_count < 0 || enable_count > 2) {
		WMT_PLAT_PR_INFO("enable_count %d is unexpected!!!\n", enable_count);
		return 0;
	}
	...
	if (enable_count == 0) {
		...
		if (ret)
			WMT_PLAT_PR_ERR("%s regulator_disable err: %d", __func__, ret);
		return ret;
	}
	...
	consys_pmic_regmap_set_value(r, MT6369_RG_LDO_VCN33_1_HW0_OP_EN_ADDR,   1 << 0, 1 << 0);
	consys_pmic_regmap_set_value(r, MT6369_RG_LDO_VCN33_1_HW0_OP_CFG_ADDR,  1 << 0, 0 << 0);
	consys_pmic_regmap_set_value(r, MT6369_RG_LDO_VCN33_1_HW0_OP_MODE_ADDR, 1 << 0, 1 << 0);
...
static INT32 consys_hw_bt_vcn33_ctrl(UINT32 enable)
{
	...
	if (enable) {
		consys_hw_wifi_bt_legacy_mode_vcn33_1_ctrl(true);
		WMT_PLAT_PR_DBG("WMT do BT PMIC on\n");
	} else {
		consys_hw_wifi_bt_legacy_mode_vcn33_1_ctrl(false);
...
static INT32 consys_hw_wifi_vcn33_ctrl(UINT32 enable)
{
	...
	if (enable) {
		...
		/* 4. set PMIC VCN33_2 LDO HW_OP_EN = 1, HW_OP_CFG = 0 */
		if (reg_VCN33_2) {
			/* set PMIC VCN33_2 LDO 3.3V */
			regulator_set_voltage(reg_VCN33_2, 3300000, 3300000);
			/* set PMIC VCN33_2 LDO SW_OP_EN = 1, SW_EN = 1, SW_LP = 0 (sw enable & normal mode) */
			regulator_set_mode(reg_VCN33_2, REGULATOR_MODE_NORMAL); /* SW_LP = 0 */
			ret = regulator_enable(reg_VCN33_2); /* SW_EN = 1 */
			if (ret)
...
#if CONSYS_PMIC_CTRL_ENABLE
	if (consys_is_rc_mode_enable_mt6855()) {
		/* set PMIC VRFIO18 LDO SW_OP_EN = 1, SW_EN = 1, SW_LP = 1 (sw ctrl into lp mode)*/
		if (reg_VRFIO18)
			regulator_set_mode(reg_VRFIO18, REGULATOR_MODE_IDLE); /* SW_LP = 1 */

		if (mtk_wcn_consys_get_adie_chipid() == PRIMARY_ADIE) {
			/* set PMIC VCN33_1 LDO SW_OP_EN = 1, SW_EN = 1, SW_LP = 1 (sw ctrl into lp mode)*/
			if (reg_VCN33_1)
				regulator_set_mode(reg_VCN33_1, REGULATOR_MODE_IDLE); /* SW_LP = 1 */
		} else {
			/* set PMIC VCN13 LDO SW_OP_EN = 1, SW_EN = 1, SW_LP = 1 (sw ctrl into lp mode)*/
			if (reg_VCN13)
				regulator_set_mode(reg_VCN13, REGULATOR_MODE_IDLE); /* SW_LP = 1 */
		}
	}
#endif
	return 0;
...
static UINT32 consys_adie_chipid_checking_flow(UINT32 adie)
{
	...
	if (reg_VRFIO18) {
		regulator_set_voltage(reg_VRFIO18, 1800000, 1800000);
		if (regulator_enable(reg_VRFIO18))
			WMT_PLAT_PR_INFO("enable VRFIO18 fail\n");
		else
			WMT_PLAT_PR_DBG("enable VRFIO18 ok\n");
	}
	udelay(300);
	...
	if (reg_VRFIO18) {
		if (regulator_disable(reg_VRFIO18))
			WMT_PLAT_PR_INFO("disable VRFIO18 fail!\n");
	}
...
{
	return dump_conn_mcu_pc_log_mt6855("");
}
...
static INT32 consys_common_dump(const char *trg_str)
{
	...
	ret += dump_conn_mcu_pc_log_mt6855(trg_str);

	ret += dump_conn_debug_dump_mt6855(trg_str);
	ret += dump_conn_mcu_debug_flag_mt6855(trg_str);
	ret += dump_conn_mcu_ahb_bus_hang_layer1_mt6855(trg_str);
	ret += dump_conn_mcu_ahb_bus_hang_layer2_mt6855(trg_str);
	ret += dump_conn_mcu_ahb_bus_hang_layer3_mt6855(trg_str);
	ret += dump_conn_mcu_ahb_bus_hang_layer4_mt6855(trg_str);
	ret += dump_conn_mcu_ahb_timeout_info_mt6855(trg_str);
	ret += dump_conn_bus_hang_debug_mt6855(trg_str);
	ret += dump_conn_mcu_apb_timeout_info_mt6855(trg_str);
	ret += dump_conn_apb_bus0_hang_mt6855(trg_str);
	ret += dump_conn_apb_bus1_hang_mt6855(trg_str);
	ret += dump_conn_apb_bus2_hang_mt6855(trg_str);
	ret += dump_conn_emi_ctrl_host_csr_mt6855(trg_str);
	ret += dump_conn_mcu_confg_emi_ctrl_mt6855(trg_str);
	ret += dump_conn_mcu_cpu_probe_mt6855(trg_str);
	ret += dump_conn_mcu_ahb_probe_mt6855(trg_str);
	ret += dump_conn_mcu_idlm_prot_prob_mt6855(trg_str);
	ret += dump_conn_mcu_wf_cmdbt_ram_prob_mt6855(trg_str);
	ret += dump_conn_mcu_pda_dbg_flag_mt6855(trg_str);
	ret += dump_conn_mcu_sysram_prb_mt6855(trg_str);
	ret += dump_conn_mcu_confg_mt6855(trg_str);
	ret += dump_conn_mcu_i_eidlm_mt6855(trg_str);
	ret += dump_conn_mcu_dma_mt6855(trg_str);
	ret += dump_conn_mcu_tcm_prob_mt6855(trg_str);
	ret += dump_conn_mcu_met_prob_mt6855(trg_str);
	ret += dump_conn_mcusys_n9_mt6855(trg_str);
	ret += dump_conn_mcu_uart_dbg_loop_mt6855(trg_str);
	ret += dump_conn_cfg_on_Debug_Signal_mt6855(trg_str);
